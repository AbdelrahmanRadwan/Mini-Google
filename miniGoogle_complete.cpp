 ...
 * to manually install the library. Ask Google about this. There is another
 * function below that will work VC++.net.
 */
...
/*
 * The DisplayResult function gets a file name and a line number, and displays 
 * this line on the console.
 */ 
...
void DisplayResult(string fileName, int lineNumber)
{
  ...
  char line[1000];
  int curLineNumber=1;
  while(curLineNumber < lineNumber)
  {
    inFile.getline(line, 1000);
    ++curLineNumber;
  }
  inFile.getline(line, 1000);
  cout<<endl<<"\nResult from ("<<fileName<< "), line (" <<lineNumber<< "): "
    <<line;
...
// the search part.  
  string searchKey;
  map< string, vector< pair<string, int> > >::iterator resultReference;
  cout<<"\n\nEnter a search word (enter . to exit): ";
  cin>>searchKey;
  while(searchKey != ".")
  {
    resultReference= index.find(searchKey);
    if( resultReference != index.end() )
    {
      vector< pair<string, int> >& resultLocations= (*resultReference).second;
      cout<<"\nFound "<< resultLocations.size() << " matches";
      for(int i=0; i< resultLocations.size(); ++i)
        DisplayResult(resultLocations[i].first,
          resultLocations[i].second);
    }
    cout<<"\n\nEnter a search word (enter . to exit): ";
    cin>>searchKey;
